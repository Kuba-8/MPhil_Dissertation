...
class TimeSeriesDataset(Dataset):
    ...
    Made the scattering transform optional to allow the class to be instantiated without it (e.g. for an ARIMA model)
...
def evaluate_forecasts(actual_values, lstm_forecasts, arima_forecasts):
    """
    Just a load of forecast evaluation metrics
    """

    lstm_mse = mean_squared_error(actual_values, lstm_forecasts)
    lstm_mae = mean_absolute_error(actual_values, lstm_forecasts)
    ...
    lstm_mape = mape(actual_values, lstm_forecasts)
    ...
    lstm_dir_acc = directional_accuracy(actual_values, lstm_forecasts)
    ...
    print(f"  MSE: {(arima_mse - lstm_mse) / arima_mse * 100:.2f}% lower with LSTM+Wavelet")
    print(f"  MAE: {(arima_mae - lstm_mae) / arima_mae * 100:.2f}% lower with LSTM+Wavelet")
...
def rolling_window_forecast_scattering_lstm(train_data, test_data, window_size, forecast_horizon, scattering_params, lstm_params, random_seed=42):
    ...
    Transform and LSTM for large datasets
    """
    ...
    optimizer = optim.Adam(lstm_model.parameters(), lr=lstm_params['learning_rate'])
    ...
    for epoch in range(lstm_params['epochs']):
        ...
        for scattering_batch, raw_batch, targets_batch in train_loader:
            ...
            outputs = lstm_model(scattering_batch, raw_batch)
        ...
        if avg_epoch_loss < best_loss:
            ...
            patience_counter = 0
            torch.save(lstm_model.state_dict(), 'best_energy_forecast_model.pt')
        else:
            patience_counter += 1
    ...
    lstm_model.load_state_dict(torch.load('best_energy_forecast_model.pt'))
    ...
    for i in range(total_forecast_points):
        ...
        with torch.no_grad():
            ...
            lstm_model.eval()
            lstm_pred_scaled = lstm_model(scattering_input, raw_input).cpu().numpy().flatten()
            
            lstm_pred = scaler.inverse_transform(lstm_pred_scaled.reshape(-1, 1)).flatten()
            lstm_forecasts.append(lstm_pred[0])
...
if __name__ == "__main__":
    ...
    metrics = evaluate_forecasts(actual_values, lstm_forecasts, arima_forecasts)

    # Simple forecast plot
    plt.figure(figsize=(12, 6))
    test_dates = test_data.index[:len(lstm_forecasts)]

    plt.plot(test_dates, actual_values, label='Actual Energy', color='blue', linewidth=2)
    plt.plot(test_dates, lstm_forecasts, label='LSTM+Wavelet Forecast', color='red', linestyle='--')
    ...
    plt.figtext(0.15, 0.15,
               f'LSTM+Wavelet RMSE: {metrics["lstm"]["rmse"]:.2f}\nARIMA RMSE: {metrics["arima"]["rmse"]:.2f}',
               bbox=dict(facecolor='white', alpha=0.8))

    plt.savefig('energy_forecast_optimized.png', dpi=300)
    plt.show()